...
@Component({
  ...
  styleUrls: ['./mens.css'] // Note plural!
})
export class Mens implements OnInit {
  products: any[] = [];
  ...
  updateFilters() {
    ...
    if (this.colorFilter.length > 0) {
      filtered = filtered.filter(product =>
        product.variants?.some((variant: any) => this.colorFilter.includes(variant.color))
      );
    }

    // Price filter
    filtered = filtered.filter(product =>
      product.variants?.some((variant: any) => variant.price <= this.maxPriceFilter)
    );
    ...
    switch (this.sortOption) {
      case 'PRICE_LOW_HIGH':
        filtered.sort(
          (a, b) =>
            (this.getCheapestVariant(a)?.price ?? 0) -
            (this.getCheapestVariant(b)?.price ?? 0)
        );
        break;
      case 'PRICE_HIGH_LOW':
        filtered.sort(
